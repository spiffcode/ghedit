...
import 'vs/css!vs/workbench/browser/media/workbench';
...
export class Workbench implements IPartService {
	...
	constructor(container: HTMLElement, workspace: IWorkspace, configuration: IConfiguration, options: IOptions, isWelcomeMode: boolean, serviceCollection: ServiceCollection,
	...
	public startup(callbacks?: IWorkbenchCallbacks): void {
		...
		try {
			...
			if (this.isWelcomeMode) {
				resolveEditorInputsPromise = TPromise.as([]);
			} else {
