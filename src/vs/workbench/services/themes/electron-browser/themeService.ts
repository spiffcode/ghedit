...
export class ThemeService implements IThemeService {
	...
	private applyThemeCSS(themeId: string, defaultId: string, onApply: (theme:IInternalThemeData) => void): TPromise<boolean> {
		let rootPath = (<IWindowConfiguration><any>this.environmentService).rootPath + '/';
...
function _applyIconTheme(data: IInternalThemeData, rootPath: string, onApply: (theme:IInternalThemeData) => void): TPromise<boolean> {
	if (!data) {
	...
	return _loadIconThemeDocument(data.path, rootPath).then(iconThemeDocument => {
		let styleSheetContent = _processIconThemeDocument(data.id, rootPath + data.path, iconThemeDocument);
...
function _loadIconThemeDocument(fileSetPath: string, rootPath: string) : TPromise<IconThemeDocument> {
// DESKTOP:	return pfs.readFile(fileSetPath).then(content => {
	return xhr({ type: 'GET', url: rootPath + fileSetPath }).then((xhr: XMLHttpRequest) => {
...
function _loadThemeDocument(themePath: string, rootPath: string) : TPromise<IThemeDocument> {
