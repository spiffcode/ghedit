...
var g_cache: IGithubTreeCache;
var g_githubService: IGithubService;
...
export class WorkspaceConfigurationService implements IWorkspaceConfigurationService, IDisposable {
	...
	constructor(
		...
		this.baseConfigurationService = new BaseConfigurationService(environmentService, eventService, githubService);
		...
		g_cache = this.cache;
		g_githubService = this.githubService;
...
function resolveContent(resource: uri): TPromise<IContent> {
	return new TPromise<IContent>((c, e) => {
		g_cache.stat(resource.fsPath, (error: any, stat?: IGithubTreeStat) => {
			if (error) {
				e('Error loading ' + resource.fsPath);
				return;
			}
			g_githubService.repo.getBlobRaw(stat.sha, (err: GithubError, content: string | boolean) => {
				if (!err) {
